...
const getData = () => {
    for(let obj of channels){
        if(obj.url.includes('.m3u8')){
            let text = `
#EXTINF:-1 tvg-id="${obj.id}" tvg-country="${CFL(obj.country)}" tvg-language="${CFL(obj.language)}" group-title="${CFL(obj.language)}" tvg-name="${obj.name}" tvg-logo="https://raw.githubusercontent.com/zuhairabs/shiachannel/main/livetv/${obj.img}",${obj.name} (1080)
${obj.url}`
            fs.appendFile('links.txt', text, err => {
  if (err) {
    console.error(err)
    return
  }
